...
**Tech Stack:** FastAPI, PostgreSQL, asyncio, Pydantic
...
backend/app/db/
├── postgres.py              # 添加 session_states 表操作
...
class SessionRepository:
    ...
    async def save_state(
        ...
        async with Database.connection.acquire() as conn:
            await conn.execute("""
...
class ErrorClassifier:
    ...
    def __init__(self, custom_rules: Dict[str, Tuple[ErrorCategory, RecoveryStrategy, int]] = None):
        """初始化分类器
...
def test_custom_rules():
    custom_rules = {"CustomAPIError": (ErrorCategory.TRANSIENT, RecoveryStrategy.RETRY_BACKOFF, 3)}
    classifier = ErrorClassifier(custom_rules)
...
from .state import ErrorCategory, RecoveryStrategy, ErrorClassification
...
import logging
from typing import Optional, Dict, Any
from .state import ErrorCategory, ErrorClassification
...
class SessionRecovery:
    ...
    async def recover(
        ...
        try:
            ...
            # 查找该用户和会话的旧状态
            async with session_repo._get_db_connection() as conn:
                row = await conn.fetchrow("""
...
- [ ] **Step 2: 在 QueryEngine.process 中包装重试循环**

在 `process` 方法开头添加重试循环逻辑：
...
async def process(
    ...
    while retry_manager.get_retry_count(conversation_id) < self._max_total_retries:
        try:
            ...
            # 允许重试，应用退避延迟
            logger.info(f"[QueryEngine] 重试 ({count}/{self._max_total_retries})")
...
- [ ] **Step 3: 在 WebSocket 连接时调用初始化**

在 `chat.py` 的 `websocket_chat_endpoint` 中：
    ...
    try:
        while True:
            ...
            if msg.type == "message" and msg.content:
                # 首次消息时执行 Step 0 初始化
                if not hasattr(websocket, '_session_initialized'):
...
git commit -m "feat(phase3): integrate session lifecycle into QueryEngine"
```
...
```bash
git add backend/app/core/session/__init__.py
git commit -m "feat(phase3): update session module exports"
```
...
```bash
git add docs/superpowers/plans/2026-04-04-session-lifecycle-plan.md
git commit -m "docs(phase3): complete session lifecycle implementation plan"
```
