...
│  │         架构层（自研 - 展示设计能力）                │   │
│  │  ┌───────────┐ ┌───────────┐ ┌───────────┐          │   │
│  │  │QueryEngine│ │IntentRouter│ │ToolRegistry│          │   │
│  │  └───────────┘ └───────────┘ └───────────┘          │   │
│  │  ┌───────────┐ ┌───────────┐ ┌───────────┐          │   │
│  │  │PromptBuild│ │ContextMgr │ │Coordinator│          │   │
│  │  └───────────┘ └───────────┘ └───────────┘          │   │
│  └─────────────────────────────────────────────────────┘   │
│                          │                                  │
│                          ▼                                  │
│  ┌─────────────────────────────────────────────────────┐   │
│  │      编排层（LangChain - 复用成熟能力）              │   │
│  │  ┌───────────┐ ┌───────────┐ ┌───────────┐          │   │
│  │  │ LLM 调用   │ │ 工具执行   │ │ 向量存储   │          │   │
│  │  │  封装      │ │  基础能力  │ │  集成      │          │   │
│  │  └───────────┘ └───────────┘ └───────────┘          │   │
...
| 层级 | 组件 | 技术选择 | 理由 |
|------|------|----------|------|
| **架构层** | QueryEngine | 自研 | 展示总控设计能力 |
| | IntentRouter（3层过滤） | 自研 | 展示意图识别架构 |
| | ToolRegistry（+元数据） | 自研 | 展示工具系统设计 |
| | PromptBuilder（分层） | 自研 | 展示提示词工程 |
| | ContextManager（压缩） | 自研 | 展示上下文管理 |
| | Coordinator（并行） | 自研 | 展示多Agent协调 |
| **编排层** | LLM 调用封装 | LangChain 0.3.x | 复用流式输出、重试逻辑 |
| | 工具执行基础 | LangChain Tools | 参数校验、错误处理 |
| | 向量存储集成 | LangChain + ChromaDB | 成熟的 RAG 能力 |
| **接入层** | 大模型 | DashScope SDK | 官方 SDK，稳定可靠 |
| | 向量数据库 | ChromaDB | 本地友好，零配置 |
| | 外部 API | 高德/和风 | 真实数据源 |

#### 设计原则

- **自研 = 展示理解**：架构层组件体现对 Agent 系统的深度理解
- **复用 = 避免重复造轮子**：编排层用成熟方案，专注业务逻辑
- **官方 = 稳定可靠**：接入层用官方 SDK，保证稳定性
